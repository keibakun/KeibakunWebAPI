...
```
PUPPETEER_EXECUTABLE_PATH="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" PUPPETEER_HEADLESS="false" npx tsx src/scripts/race/MainRaceResult.ts 2025 10
```

**主要スクリプト（場所: src/scripts）**
- `MainRaceSchedule.ts` : 年を指定して1年分の開催スケジュールを取得します。
	- 実行例: `npx tsx src/scripts/race/MainRaceSchedule.ts 2025`
- `MainRaceList.ts` : 開催日（kaisaiDate）ごとのレース一覧を取得します。
	- 実行例: `npx tsx src/scripts/race/MainRaceList.ts 2025 6`
- `MainShutuba.ts` : 指定年月から開催日を取得し、各レースの出馬表を取得します。
	- 実行例: `npx tsx src/scripts/race/MainShutuba.ts 2025 6`
- `MainRaceResult.ts` : RaceList から raceId を取得してレース結果を取得します。
	- 実行例: `npx tsx src/scripts/race/MainRaceResult.ts 2025 6`
- `main_horseDetail.ts` : Shutuba から抽出した horseId を使って馬の詳細を取得します。
	- 実行例: `npx tsx src/scripts/horseDetail/main_horseDetail.ts 2025 6`
