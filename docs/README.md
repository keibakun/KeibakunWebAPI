...
```
PUPPETEER_EXECUTABLE_PATH="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" PUPPETEER_HEADLESS="false" npx tsx src/scripts/race/main_raceResult.ts 2025 10
```

**主要スクリプト（場所: src/scripts）**
- `main_raceSchedule.ts` : 年を指定して1年分の開催スケジュールを取得します。
	- 実行例: `npx tsx src/scripts/race/main_raceSchedule.ts 2025`
- `main_raceList.ts` : 開催日（kaisaiDate）ごとのレース一覧を取得します。
	- 実行例: `npx tsx src/scripts/race/main_raceList.ts 2025 6`
- `main_shutuba.ts` : 指定年月から開催日を取得し、各レースの出馬表を取得します。
	- 実行例: `npx tsx src/scripts/race/main_shutuba.ts 2025 6`
- `main_raceResult.ts` : RaceList から raceId を取得してレース結果を取得します。
	- 実行例: `npx tsx src/scripts/race/main_raceResult.ts 2025 6`
