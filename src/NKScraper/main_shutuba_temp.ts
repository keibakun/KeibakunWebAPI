...
export class Main_Shutuba {
    ...
    /**
     * ワーカープール: 250件ごとにバッチ処理し、各バッチ完了後に1時間待機する
    ...
    private async runWorkerPool(pages: Page[], raceIds: string[]): Promise<void> {
        ...
        const WAIT_MS = 60 * 60 * 1000; // 1時間
        const total = raceIds.length;

        for (let batchStart = 0; batchStart < total; batchStart += BATCH_SIZE) {
            const batch = raceIds.slice(batchStart, batchStart + BATCH_SIZE);
            const batchEnd = Math.min(batchStart + BATCH_SIZE, total);
            logger.info(`バッチ処理開始: ${batchStart + 1}〜${batchEnd} 件目 / 全${total}件`);
            ...
            const worker = async (page: Page, workerId: number) => {
                while (true) {
                    ...
                    logger.info(`[Worker${workerId}] (${globalIdx + 1}/${total}) レースID: ${raceId} の出馬表を取得します`);
                    try {
            ...
            logger.info(`バッチ処理完了: ${batchStart + 1}〜${batchEnd} 件目`);

            if (batchEnd < total) {
                logger.info(`次のバッチまで1時間待機します...`);
                await this.sleep(WAIT_MS);
                logger.info(`待機完了。次のバッチを開始します。`);
            }
        }
    }
