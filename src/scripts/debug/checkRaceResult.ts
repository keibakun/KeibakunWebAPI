/**
 * RaceResult フォルダの過不足調査スクリプト
 *
 * 処理概要:
 * 1. 2015〜2026年の各年月について RaceList から raceId 一覧を収集
 *    → temp/work/raceResult/target/{year}{MM}.json に一時保存（処理後削除）
 * 2. 各 raceId に対して RaceResult/{year}/{month}/{rest}/index.html の存在と
 *    result プロパティが空でないことを確認
...
const RACE_RESULT_ROOT = path.join(ROOT, "RaceResult");
const TARGET_DIR = path.join(ROOT, "temp/work/raceResult/target");
...
/**
 * raceId に対応する RaceResult ファイルをチェックする
 * @returns 問題の種別: "missing"（ファイルなし）| "empty_result"（resultが空）| null（正常）
 */
async function checkRaceResult(raceId: string): Promise<"missing" | "empty_result" | null> {
    const year = raceId.substring(0, 4);
    const month = raceId.substring(4, 6);
    const rest = raceId.substring(6);
    const filePath = path.join(RACE_RESULT_ROOT, year, month, rest, "index.html");
...
async function main(): Promise<void> {
    ...
    for (let year = START_YEAR; year <= END_YEAR; year++) {
        for (let month = 1; month <= 12; month++) {
            ...
            // ② RaceResult を走査して問題を収集
            const problems: ProblemEntry[] = [];
            for (const raceId of raceIds) {
                const reason = await checkRaceResult(raceId);
                if (reason !== null) {
    ...
    console.log("temp/work/raceResult/target を削除しました。");
}
